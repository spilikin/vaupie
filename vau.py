...
import io
import logging
from vaupy.init import init_endpoint
from vaupy.request import post_request
from vaupy.jwt import create_access_token
...
def authorization(state: State):
    sys.stdout.write (f"Authorization:{state.endpoint.auth_token}")
...
if args.action == "init" and args.token == None:
    args.token = create_access_token(open('../ref-eRp-FD-Server/idp-private.pem').read())
...
if args.action == "init":
    ...
    if args.action == "req":
        ...
        access_token = create_access_token(open('../ref-eRp-FD-Server/idp-private.pem').read())
        print(access_token)
    else:
